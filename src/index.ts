...
import {
  GraphData,
  GraphResponse,
} from "./interfaces/librelink/graph-response";
import {
  AuthTicket,
  Connection,
  GlucoseItem,
} from "./interfaces/librelink/common";
import { getUtcDateFromString, mapTrendArrow } from "./helpers/helpers";
import {
  LibreLinkUpHttpHeaders,
  NightScoutHttpHeaders,
} from "./interfaces/http-headers";
...
const NIGHTSCOUT_DEVICE_NAME =
  process.env.DEVICE_NAME || "nightscout-librelink-up";
...
async function main(): Promise<void> {
  if (!hasValidAuthentication()) {
    ...
    if (!authTicket) {
      logger.error(
        "LibreLink Up - No AuthTicket received. Please check your credentials."
      );
      deleteAuthTicket();
      return;
    }
  ...
  try {
    ...
    try {
      if (response.data.status !== 0) {
        logger.error(
          `LibreLink Up - Non-zero status code: ${JSON.stringify(
            response.data
          )}`
        );
        return null;
      ...
      logger.error(
        "Invalid authentication token. Please check your LibreLink Up credentials",
        err
      );
      return null;
    }
...
export async function getLibreLinkUpConnection(): Promise<string | null> {
  try {
    ...
    if (!process.env.LINK_UP_CONNECTION) {
      logger.warn(
        "You did not specify a Patient-ID in the LINK_UP_CONNECTION environment variable."
      );
      logPickedUpConnection(connectionData[0]);
      return connectionData[0].patientId;
    }

    const connection = connectionData.filter(
      (connectionEntry) =>
        connectionEntry.patientId === process.env.LINK_UP_CONNECTION
    )[0];
    if (!connection) {
...
export async function createFormattedMeasurements(
  measurementData: GraphData
): Promise<Entry[]> {
  const formattedMeasurements: Entry[] = [];
  const glucoseMeasurement = measurementData.connection.glucoseMeasurement;
  //   const measurementDate = getUtcDateFromString(
  //     glucoseMeasurement.FactoryTimestamp
  //   );
  ...
  if (lastEntry === null || measurementDate > lastEntry) {
    if (glucoseMeasurement !== null) {
      if (
        lastEntry !== null &&
        getUtcDateFromString(glucoseMeasurement.FactoryTimestamp) > lastEntry
      ) {
        ...
        logger.info("Looks to already be in the system - Skipping");
      }
      logger.info("Pushing latest value: " + glucoseMeasurement.ValueInMgPerDl);
    } else {
      logger.info("Gluecose measurement is null");
    }
  }

  measurementData.graphData.forEach(
    (glucoseMeasurementHistoryEntry: GlucoseItem) => {
      const entryDate = getUtcDateFromString(
        glucoseMeasurementHistoryEntry.FactoryTimestamp
      );
...
async function uploadToNightScout(measurementData: GraphData): Promise<void> {
  const formattedMeasurements: Entry[] = await createFormattedMeasurements(
    measurementData
  );
  ...
  if (formattedMeasurements.length > 0) {
    logger.info(
      "Trying to upload " +
        formattedMeasurements.length +
        " glucose measurement items to Nightscout"
    );
    try {
      const url = getNightscoutUrl() + "/api/v1/entries";
      const response = await axios.post(url, formattedMeasurements, {
        headers: nightScoutHttpHeaders,
      });
      if (response.status !== 200) {
        ...
        logger.info(
          "Upload of " +
            formattedMeasurements.length +
            " measurements to Nightscout succeeded"
        );
      }
    } catch (error) {
  ...
  connectionData.map((connectionEntry: Connection, index: number) => {
    logger.debug(
      "[" +
        (index + 1) +
        "] " +
        connectionEntry.firstName +
        " " +
        connectionEntry.lastName +
        " (Patient-ID: " +
...
function logPickedUpConnection(connection: Connection): void {
  logger.info(
    "-> The following connection will be used: " +
...
function getLluAuthHeaders(): LibreLinkUpHttpHeaders {
  ...
  logger.debug(
    "authenticatedHttpHeaders: " + JSON.stringify(authenticatedHttpHeaders)
  );
